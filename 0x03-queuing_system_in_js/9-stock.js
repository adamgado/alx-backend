...
const getItemById = (id) => {
  ...
  if (item) {return Object.fromEntries(Object.entries(item));}};
const app = express();
const PORT = 1245;
const cl = createClient();
const reserveStockById = async (itemId, stock) => {return promisify(cl.SET).bind(cl)(`item.${itemId}`, stock);};
const getCurrentReservedStockById = async (itemId) => {return promisify(cl.GET).bind(cl)(`item.${itemId}`);};
...
app.get('/list_products/:itemId(\\d+)', (req, res) => {
  ...
  if (!productItem) {res.json({ status: 'Product not found' }); return;}
...
const resetProductsStock = () => {
  return Promise.all(
    listProducts.map(item => promisify(cl.SET).bind(cl)(`item.${item.itemId}`, 0),));};
app.listen(PORT, () => {
  resetProductsStock()
    .then(() => {
